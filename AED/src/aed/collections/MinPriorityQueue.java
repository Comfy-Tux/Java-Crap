...
/*
   ...
   //max priority queue helper data structure

   private class MaxPQ{
      private UndirectedEdge[] pq;
      private int N = 0;

      public MaxPQ(int maxN){ pq = new UndirectedEdge[maxN+1];}

      public boolean isEmpty(){return N == 0;}

      public int size(){return N;}

      public void insert(UndirectedEdge v){
         pq[++N] = v;
         swim(N);
      }

      public UndirectedEdge delMax(){
         UndirectedEdge max = pq[1];
         exch(1,N--);
         pq[N+1] = null;
         sink(1);
         return max;
      ...
      private void exch(int i,int j){
         UndirectedEdge t = pq[i]; pq[i] = pq[j]; pq[j] = t;
      }
